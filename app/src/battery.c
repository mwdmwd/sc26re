...
static int battery_poll_once(struct controller_battery_report *report)
{
	...
	err |= mp2733_update_reg(MP2733_REG_ADC_OTG_CONFIGURATION, MP2733_ADC_RATE, 0);
	k_msleep(BATTERY_ADC_SETTLE_MS);
	err |= mp2733_update_reg(MP2733_REG_ADC_OTG_CONFIGURATION, 0, MP2733_ADC_RATE);
	err |= mp2733_update_reg(MP2733_REG_TIMER_CONFIGURATION, 0, MP2733_EN_ADC_DSG);
	if(err)
	{
		return err;
	}
