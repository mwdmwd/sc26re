...
enum
{
	...
	ESB_FEATURE_GET_VERSION_ATTRIBUTE = 0xf2,
	ESB_FEATURE_SETTINGS_READ = 0xed,
};
...
static bool feature_request_expects_response(const uint8_t *request, size_t request_len)
{
	...
	switch(request[0])
	{
		...
		case ID_GET_CHIPID:
		case ESB_FEATURE_GET_VERSION_ATTRIBUTE:
		case ESB_FEATURE_SETTINGS_READ:
			return true;
		default:
			return false;
	}
